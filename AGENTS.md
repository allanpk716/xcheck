...
0.17.0 起支持**夜链**:`/xcheck` 入口一问选"自动推进"(或 `--night` 免问)后,无人值守贯通"评审 → to-spec 固化 → to-tickets 拆票 → 逐票 TDD 实施 + 双轴评审(worktree 内执行)";自动化优先级最高(下游技能关卡自动裁定)。0.18.0 起夜链**自动推送保进度**:每票完成即推分支,收工自动开 PR(base = 主仓当前分支,只保进度不自动合并)、主仓当前分支的 spec/票提交直推;绝不 force push;收尾三层输出(对话/通知/晨报)统一「夜链结论 → 简报 → 推荐下一步」骨架,通知带【项目名】title 与 PR 链接。
...
flow.md(主会话执行,严格按步走)
  ...
  第 11 步 夜链(NIGHT_MODE=1,终态收尾后):建 NIGHT.md → 终态分流(结论性终态
             ...
             记账,每票完成即推分支 → 终局全分支评审(一轮修复)→ 收工推送(主仓
             当前分支直推 + 开 PR,gh→tea 降级,幂等,绝不 force)→ 晨报 MORNING.md
             (四问 + 文末裁定与停靠附录)+ 三节点通知(title=【项目名】,中文走 UTF-8
             文件防 GBK 问号,见 flow.md 通知纪律)+ 对话终局播报(夜链结论/简报/
             推荐下一步 三段式)+ NIGHT 勾 finish(头部记 push/pr)
```
...
9. **夜链安全栏与自动化优先级(--night / 自动推进)**:第 11 步下游执行的一切代码改动只在 worktree;**对外动作白名单只有三类——每票完成即推夜链分支、收工开 PR(base = 主仓当前分支,只保进度)、主仓当前分支收工直推 spec/票等文档性提交;白名单之外仍全禁:绝不 force push、不自动合并 PR、不上 issue tracker、不动主工作区**(spec 与票文件在主仓落盘+commit 是白名单的配套例外,含为放行票目录对宿主 .gitignore 的最小修改);推送/PR 失败不阻塞,通知+晨报记账;下游技能(to-spec/to-tickets/修复环)确认关卡自动裁定记账,唯一停链例外 = 不可逆或破坏性/安全敏感/出 worktree 副作用/全盘皆猜;通知失败不阻塞,晨报兜底;夜间会话须免弹窗权限模式。
...
- **夜链账本**:PROGRESS 头部可选字段 `night = on`(夜链才写);NIGHT.md 四阶段 `review/plan/impl/finish`(`plan` = to-spec 固化 + to-tickets 拆票;`impl` = 逐票实施,票级台账逐票记账;`finish` 未勾 = 夜链未完成,不进 PROGRESS 阶段枚举),头部 `push`/`pr` 字段记收工推送与 PR 结果(0.18.0)。**夜链结论四值**(收尾对话/通知/PR/晨报共用):`全绿 / 带停靠完成 / 未完成 / 失败收工`;短稿终态(没写代码)不记四值,记"未执行实施(<原因>)"。
