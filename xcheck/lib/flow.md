...
- **MODE = auto**(壳没定:输入为空或含糊)→ 按 `~/.claude/skills/xcheck/lib/context-intake.md` 第 0.0 步**对象解析器**执行:解析阶梯(文件型 > 讨论型 > 诊断型 > 反问)从最近对话推断**评审对象 + 模式 + 背景原话**,弹**一个**确认窗打包过目。确认后落定 MODE、落内容文件、建 `<ts>/` 目录与 PROGRESS 头部(`source` = 解析出的文件路径 → 修订将落原文件同目录;固化稿 → `inline`),继续第 1 步。
