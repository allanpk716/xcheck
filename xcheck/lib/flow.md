...
票 03: blocked(票02)
票 04: rework(修复中,R1) → complete(...,retries=2)   # retries=k 尾注只记实施位、重试过(k>0)才记
票 05: impl retry 0/5(超时,已还原,检出序号=3)        # 重试追记行(非状态):首行还原完成后落
票 05: impl retry 1/5(超时)                          # 此后每次重试实际派发时追记,k≥1 沿用首行 m
票 06: paused(重试耗尽:5×失败,末次原因=超时,阶段=review)
终局review retry 0/5(超时,检出序号=9)               # 终局评审非票记法;耗尽→waiting(终局评审重试耗尽,…),finish 不勾
```

**恢复语义**:finish未勾时核验0.22字段集与所需产物再续;旧字段集(0.21及更早)拒绝自动续跑。plan未勾先读现有spec/票,不盲目覆盖用户修改。impl从票账本重算就绪集;complete须提交可达+验证记录,账本无complete提交但路径有残留的票先按"失败票路径还原"清理;重试等待中的单元(已还原未重派)断链重建:计数从追记行重建不重置,按已耗 k 取下一档**整档重等**(不折算档内进度),错峰按各自首行检出序号升序重赋 i;paused/blocked保留解除条件,不能以有行即跳过。有paused/blocked或全局待决则waiting记解除条件,finish保持未勾;再调用先核是否已有新证据,没有则直接返回暂停摘要,不重复发布/通知。
...
| 票台账行 | NIGHT | complete(oid=,rounds=,tests=,review=) \| paused(原因,解除条件) \| paused(重试耗尽:N×失败,末次原因,阶段=impl\|review) \| blocked(票号) \| rework(轮次) \| committed-unreviewed(oid);另含重试追记行(非新状态):`票 NN impl/review retry k/N(<原因>)`、`终局review retry k/N(<原因>)`;首行 impl 带"已还原,检出序号=m",review/终局带"检出序号=m" | 逐票/逐次重试 | 状态机;complete 落定且实施侧重试过(k>0)尾注 `retries=k`,只记实施位 |
...
| 夜链任何一步失败(spec 固化崩/拆票崩/逐票实施崩/通知崩) | 推通知(停在哪、早上怎么续);NIGHT.md 停在当前阶段,`/xcheck --night` 可续(impl 段从票账本重算就绪集);通知失败不阻塞,晨报兜底;泳道派发单元(实施/票级评审/终局review)失败走有界重试(11.6),耗尽才 paused |
