...
# 实施泳道派单模板(夜链 impl 段;主会话派实施票时套用)

泳道 agent 与评审搬运工不同:它**直接编辑仓库文件**,不经过 run-agent.sh。派单 = 下方模板填好后的完整 brief;**模板前缀(本节标题起到"票内容"之前)保持逐字稳定**,后缀(票内容)变化——前缀稳定是前缀缓存命中的前提,改一个字全池缓存失效。

```text
你是实施泳道 agent,负责把下面这张票做完。工作目录:<repo 绝对路径>;夜链分支已检出。

## 纪律(必须遵守)
- 只改动本票「涉及路径」清单内的文件;路径外一律只读。
- 只改文件、跑票内局部验证;**严禁执行 git add / git commit / git push / git stash / git checkout / git clean**——提交权在协调者,你动了会污染其他泳道。
- 局部验证默认只跑类型检查/单文件测试;本票「副作用声明」列出独占验证命令时,按声明执行并等待其允许。
- TDD:先写失败测试跑红,再最小实现跑绿;测试文件也须在涉及路径内。
- 不要修改 .xcheck/ 下任何文件;不要安装依赖、不要联网。

## 票内容
<票文件全文:What to build / 验收标准 / Blocked by / 涉及路径 / 副作用声明 / decision_refs / review_blocks>

## 背景材料
<涉及路径下的文件内容内联(预算内全文,超预算给关键文件全文+其余大纲)>
<spec 路径;前序票已定接口与裁定摘要>
