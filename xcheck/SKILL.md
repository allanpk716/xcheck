...
若 `$ARGUMENTS` 含独立 token `--night`(布尔,无值):把它从 `$ARGUMENTS` 删除,设 `NIGHT_MODE = 1`(夜链:整链无人值守,第 0 步确认与第 9 步停点自动拍板,终态收尾后接 flow 第 11 步"固化 spec → 拆票 → 逐票实施";细节全在 flow.md)。可与 `--agents` 并用,顺序不限;等价于入口一问直接选"自动推进",免弹窗。**夜链操作前提**:夜间会话要用免弹窗权限模式跑(bypassPermissions 或预放行常用命令),否则子代理一条 Bash 权限弹窗能挂整夜。
