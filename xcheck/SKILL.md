...
若 `$ARGUMENTS` 含独立 token `--night`(布尔,无值):把它从 `$ARGUMENTS` 删除,设 `NIGHT_MODE = 1`(夜链:整链无人值守,第 0 步确认与第 9 步停点自动拍板,终态收尾后接 flow 第 11 步"固化 spec → 拆票 → 逐票实施",含自动推送与收工开 PR;细节全在 flow.md)。可与 `--agents` 并用,顺序不限;等价于入口一问直接选"自动推进",免弹窗。**夜链操作前提**:夜间会话要用免弹窗权限模式跑(bypassPermissions 或预放行常用命令),否则子代理一条 Bash 权限弹窗能挂整夜。
...
- **都没有** → 非夜链(`NIGHT_MODE` 未设)→ **入口一问**(AskUserQuestion,单选,自动推进列第一):
  - `自动推进到底(推荐) —— 评审→把结论写成实施说明书→拆成小任务→逐个写代码并互查,全程不问你;改动在隔离分支上,每完成一票自动推远端,收工自动开 PR(不自动合并);手机有通知,早上看晨报` → 设 `NIGHT_MODE = 1`,进第 3 件。
  - `正常交互 —— 老流程:评完给三类清单,停点你拍板;评完即止,不接着写代码` → 不设,进第 3 件。
...
- **有且无 NIGHT_MODE** → AskUserQuestion(单选):
  - `续跑 <ts>(停于:<第一个未勾阶段名 + 一句人话>,如"verify——逐条核实证据"/"impl——逐票写代码")` → 评审段未完:**设 `RESUME_TS = <ts>`**,**跳过路由与摄入**,按第 4 件转派 flow.md 恢复模式;评审段已终态(夜链续跑):**不设 RESUME_TS,一并设 `NIGHT_MODE = 1`**——弹窗确认即为人工签字,按第 4 件转派 flow 第 11 步。
  - `新开` → **先走上面的入口一问**(自动推进/正常交互),再进第 3 件(输入为空也没关系,第 3 件会转解析器从上下文推断)。
...
- 产物落 `.xcheck/<ts>/`(已 gitignore);对话说人话、文件留机器账(机器词只进 SUMMARY.md,进对话必须翻译);终点=把三类清单(查实的/可实验的/存疑的)分好、附在被评文档后交下游,**不是把方案修到完美**——带着清单进开发是一等公民出口;交付按第 9 步四问结构(这次评了什么/发现了什么/改了什么/你现在要决定什么)说全且自包含,不让用户翻文件才能懂;终态收尾原样输出"**以上是建议,共识 ≠ 正确,最终你拍板。**"
