...
4. 每个 subagent 的指令 = `~/.claude/skills/xcheck/lib/subagent-carrier.md`(填好该 agent 的 AGENT_NAME / PROMPT_FILE / RESULT_SHAPE=diag 结构;CLI 命令与超时由 lib/run-agent.sh 从 agents.toml 解析,主会话不取参)。
...
- 超时/挂起判定/击杀已由 `lib/run-agent.sh` 统一机械化(所有 agent 都有硬超时笼子,needs_timeout 只是"历史会卡"的提示标记)。
